...
`drift-data-ingest-gate` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies ingest behavior through round-trip fixtures, with lossless normalization checks and no credentials or hosted services.
...
The score is deterministic. The current threshold is 178. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 6.
